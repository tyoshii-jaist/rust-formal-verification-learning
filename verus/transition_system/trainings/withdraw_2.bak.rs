...
tokenized_state_machine!{VBQueue<T> {
    fields {
        ...
        #[sharding(variable)]
        pub prod_checkout_idx: Option<nat>,

        #[sharding(variable)]
        pub cons_checkout_idx: Option<nat>,
    }
    ...
    pub open spec fn is_checked_out(&self, i: nat) -> bool {
        match (self.prod_checkout_idx, self.cons_checkout_idx) {
            (Option::Some(pidx), Option::Some(cidx)) => pidx != i && cidx != i,
            (Option::Some(pidx), Option::None) => pidx != i,
            (Option::None, Option::Some(cidx)) => cidx != i,
    ...
    pub fn in_bounds(&self) -> bool {
        &&& 0 < self.len()
        &&& match self.prod_checkout_idx {
            Option::Some(pidx) => pidx >= 0 && pidx < self.len(),
            Option::None => true,
        }
        &&& match self.cons_checkout_idx {
            Option::Some(cidx) => cidx >= 0 && cidx < self.len(),
            Option::None => true,
        }
    }

    #[invariant]
    pub fn no_duplicate_checkout(&self) -> bool {
         match (self.prod_checkout_idx, self.cons_checkout_idx) {
            (Option::Some(pidx), Option::Some(cidx)) => pidx != cidx,
            (_, Option::None) => true,
            (Option::None, _) => true,
        }
    }
    ...
    init!{
        initialize(backing_cells: Seq<CellId>, storage: Map<nat, cell::PointsTo<T>>) {
            ...
            init prod_checkout_idx = None;
            init cons_checkout_idx = None;
        }
    }
    ...
    transition!{
        prod_checkout_at(idx: nat) {
            ...
            assert(match pre.prod_checkout_idx {
                Option::Some(pidx) => idx != pidx,
                Option::None => true,
            });
            ...
            update prod_checkout_idx = Option::Some(idx);
    ...
    transition!{
        cons_checkout_at(idx: nat) {
            ...
            assert(match pre.cons_checkout_idx {
                Option::Some(cidx) => idx != cidx,
                Option::None => true,
            });
            ...
            update cons_checkout_idx = Option::Some(idx);
