...
{
    pub tracked perm: PCell<BufferPermission>,
    pub tracked token: PointsToRawExample::grant_state,
}

impl GhostBufferPermission
{
    pub open spec fn wf(self, inst: PointsToRawExample::Instance, cell: PCell<BufferPermission>) -> bool {
        &&& self.perm@.id == cell.id()
        &&& self.token.instance_id() == inst.id()
        &&& self.perm@.value as nat == self.token.value()
    }
}
...
struct_with_invariants!{
    pub struct ExBuffer {
        ...
        points_to_raw_inv: Tracked< Shared<AtomicInvariant<_, GhostBufferPermission, _>> >,
        ...
        cell_buffer_permission: Tracked<PCell<BufferPermission>>,
    ...
    pub closed spec fn wf(&self) -> bool {
        predicate {
            ...
            &&& self.split_inv@@.namespace() != self.points_to_raw_inv@@.namespace()
        }

        invariant on points_to_raw_inv with (instance, cell_buffer_permission)
            specifically (self.points_to_raw_inv@@)
            is (v: GhostBufferPermission)
        {
            v.wf(instance@, cell_buffer_permission)
        }
...
impl ExBuffer
{
            ...
            r.consumer@->Some_0.value().grant is None,
    {
        ...
        let tracked points_to_raws = BufferPermission {
            pool: points_to_raw,
            prod: PointsToRaw::empty(buffer_ptr@.provenance),
            cons: PointsToRaw::empty(buffer_ptr@.provenance),
        };
        let (points_to_raws_cell, Tracked(points_to_raws_cell_perm)) = PCell<BufferPermission>::new();
        let tracked ghost_buffer_perm = GhostBufferPermission {
            perm: points_to_raws_cell_perm,
            token: grant_state_token,
        },
        let tracked points_to_raw_inv = AtomicInvariant::new(tr_inst, buffer_perm_token, 0);
        let tracked points_to_raw_inv = Shared::new(points_to_raw_inv); // Shared は Ghost object を中に入れて、duplicate して &T を取り出すことができる。
