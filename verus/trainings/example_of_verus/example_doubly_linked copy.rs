...
mod doubly_linked_list {
    ...
    pub struct DoublyLinkedList<V> {
        ...
        ghost_state: Tracked<GhostState<V>>, // GhostState は下で定義されている。
    }
    ...
    impl<V> DoublyLinkedList<V> {
        ...
        // &&& は結合順位が低い &&
        // https://verus-lang.github.io/verus/guide/operators.html?highlight=logical%20and#logical-implication
        spec fn well_formed_node(&self, i: nat) -> bool {
            &&& self.ghost_state@.points_to_map.dom().contains(i) // dom は Map の keys を set として返す。i 番目があることの確認。
            &&& self.ghost_state@.points_to_map[i].pptr() == self.ghost_state@.ptrs[i as int] // map と seq の対応が取れていることの確認。
            &&& self.ghost_state@.points_to_map[i].mem_contents() matches MemContents::Init(node) // MemoCOntents::Init(node) は node をポイントする形で初期化済み(有効なデータを持つ)を示す
                  && node.prev == self.prev_of(i) && node.next == self.next_of(i) // matches の書き方はこちら https://verus-lang.github.io/verus/guide/datatypes_enum.html?highlight=matches#matches-with---and
        }
